...
def cancel_roi(frame):
	...
	mask[int(height * ROI_Y[0]) : int(height * ROI_Y[1]), int(width * ROI_X[0]) : int(width * ROI_X[1])] = 1
...
def get_background(container):
	...
	print(f"number of frame: {container.streams.video[0].frames}/{COUNT_FOR_BACKGROUND} = {sampling}", end = "")
