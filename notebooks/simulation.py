...
ATTENUATION = 20 # attenuation coefficient in dB / m, 10: between 100-200 degrees, 4000+ Hz
...
def get_df_theory(frequencies, distances, gain=1.0, chosen_mics=range(4)):
    ...
    for k, distance in enumerate(distances):
        ...
        H[:, :, k] = get_slice_theory(mic_positions[chosen_mics], source, frequencies, gain=gain)
    return H
...
def get_freq_slice_pyroom(frequencies, distance_cm, yaw_deg=0, signal=None):
    ...
    mag = get_average_magnitude(room, signal, n_buffer=N_BUFFER, n_times=n_times) # n_mics x n_frequencies
