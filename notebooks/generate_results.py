...
def generate_signals_pyroom(source, source_signal, mics_rotated, time, noise=0, ax=None):
    ...
    physics = pra.parameters.Physics().get_sound_speed()
    speed_of_sound = physics.get_sound_speed()
...
def inner_loop(mics_drone, degrees, times_noisy, signals_f_list, signals_f_multimic, frequencies):
    ...
    beam_former.init_dynamic_estimate(combination_n=len(mics_clean), 
...
if __name__ == "__main__":
    ...
    inner_loop(mics_drone, degrees, times, signals_f_list, signals_f_multimic, frequencies)
