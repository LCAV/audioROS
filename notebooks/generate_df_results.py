...
def data_collector_from_df(df_all, exp_name, mic_type, motors):
    ...
    if PLATFORM == "epuck":
        from epuck_description_py.experiments import WALL_OFFSET_M_STEPPER
    elif PLATFORM == "crazyflie":
        from crazyflie_description_py.experiments import WALL_OFFSET_M_STEPPER
    else:
    ...
    if not OVERWRITE_RAW:
        ...
        with progressbar.ProgressBar(max_value=max_index) as p:
            for i_row, row in df_filtered.iterrows():
                row.distance += WALL_OFFSET_M_STEPPER * 100
