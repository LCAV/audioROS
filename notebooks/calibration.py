...
def get_calibration_function(plot=False):
    ...
    calib_function = interp1d(x=calib_f, y=calib_psd, kind='linear', fill_value='extrapolate')
    uniform_psd = calib_function(uniform_f)

    if plot:
        fig, axs = plt.subplots(1, row.psd.shape[0], squeeze=False, sharey=True, sharex=True)
