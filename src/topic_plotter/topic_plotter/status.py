...
class StatusPlotter(Node):
    def __init__(self):
        ...
        self.hline = self.ax.axhline(ReaderCRTP.BATTERY_OK, label='OK level')
        self.ax.scatter([], [], color="C0", label='current level')
        self.ax.legend()
        self.fig.set_size_inches(14, 20)
        plt.show(block=False)

    def listener_callback_status(self, msg):
        self.ax.scatter(msg.timestamp/1000, msg.vbat, color="C0")
